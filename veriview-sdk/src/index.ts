...
export class VeriView {
    ...
    private config: Required<VeriViewConfig>;
     ...
     * @private
     */
    private mapToSecurityReport(data: GatewayResponse): SecurityReport {
        const elements: VeriViewElement[] = data.interactive_elements.map((el) => ({
            vv_id: el.vv_id,
            tag: el.tag,
            text: el.text,
        }));
     ...
     * @private
     */
    private handleError(error: unknown, url: string): SecurityReport {
